...
namespace csl {
  ...
  public:
    array() : _size() {}
    array(int n, _Tp x = _Tp()) : _size(n) { while (n > 0) _data[--n] = x; }
  public:
    inline _Tp* data() { return _data; }
    inline const _Tp* data() const { return _data; }
    inline int size() const { return _size; }
    inline void clear() { _size = 0; }
    inline _Tp& operator [](int x) { return _data[x]; }
    inline const _Tp& operator [](int x) const { return _data[x]; }
    inline _Tp& at(int x) { return _data[x]; }
    inline void resize(int x)
    { if (x <= _size) _size = x; else while (_size < x) _data[_size++] = _Tp(); }
    inline void push_back(_Tp x) { _data[_size++] = x; }
  ...
  public:
    ...
    BigDec() : _bits(1, 0) {}
    BigDec(int32 value) { split(_bits, value); }
    BigDec(uint32 value) { split(_bits, value); }
    BigDec(int64 value) { split(_bits, value); }
    BigDec(uint64 value) { split(_bits, value); }
    BigDec(const container& value) : _bits(value) {}
    BigDec(container&& value) : _bits(std::move(value)) {}
    explicit BigDec(const char *p) {
      int n = strlen(p); _bits.resize((n - 1) / Bit + 1);
      for (int i = n, j = 0, k = 1; i > 0; --i)
      { _bits[j] += (p[i - 1] - '0') * k; if ((k *= 10) == Base) k = 1, ++j; }
    }
    inline uint32* data()
    { return _bits.data(); }
    ...
    { AddSelf(_bits, value._bits); return *this; }
    inline _Self& operator -= (const _Self& value)
    { SubSelf(_bits, value._bits); return *this; }
    inline _Self& operator *= (const _Self& value)
    { _bits = Multi(_bits, value._bits); return *this; }
    inline _Self& operator /= (const _Self& value)
    { container r; Divide(_bits, value._bits, r); return _bits = std::move(r), *this; }
    inline _Self& operator %= (const _Self& value)
    { container r; Divide(_bits, value._bits, r); return *this; }
  ...
  private:
    ...
    { bits.clear(); do { bits.push_back(value % Base), value /= Base; } while (value); }
    static void StripLeadingZero(container& bits)
    { int32 l = bits.size(); while (l > 1 && bits[l - 1] == 0) --l; bits.resize(l); }

    static int Compare(const container& l, const container& r) {
      int32 ll = l.size(), rl = r.size();
      if (ll ^ rl) return (ll < rl) ? -1 : 1;
      for (int i = ll - 1; i >= 0; i--) if (l[i] ^ r[i]) return (l[i] < r[i]) ? -1 : 1;
      return 0;
    }
    ...
    static void AddSelf(container& l, const container& r) {
      ...
      for (int32 i = 0; i < ll; ++i) {
        c = c + l[i]; if (i < rl) c = c + r[i];
        if (c < Base) l[i] = c, c = 0; else l[i] = c - Base, c = 1;
      }
      if (c) l.push_back(c);
    }
    ...
    static void SubSelf(container& l, const container& r) {
      ...
      for (int32 i = 0; i < ll; ++i) {
        c = c + l[i]; if (i < rl) c = c - r[i];
        if (c < 0) l[i] = c + Base, c = -1; else l[i] = c, c = 0;
      }
      StripLeadingZero(l);
    }
    ...
    static container Multi(const container& l, const container& r) {
      ...
      for (int32 i = 0; i < ll; i++) {
        ...
        for (int32 j = 0; j < rl || c > 0; j++) {
          ...
          int32 k = i + j;
          if (k < bl) c += b[k]; else b.resize(bl = k + 1);
          b[k] = c % Base, c /= Base;
        }
      }
    ...
    static void Divide(container& l, const container& r, container& q) {
      ...
      for (int32 i = ll - 1; i >= 0; --i) {
        ...
        while (left < right) {
          ...
          if (Compare(Multi(r, container(1, mid)), c) <= 0) left = mid; else right = mid - 1;
        }
