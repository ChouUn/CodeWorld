...
namespace csl {
  template< typename T = long long, typename _Sequence = std::vector<T> >
  struct ntt {
  public:
    ...
    ntt(size_type n) {
      for (P = (1 << n) + 1; ; P += 1 << n) {
        bool b = true;
        for (T j = 2; j * j < P; j++)
          if (P % j == 0) { b = false; break; }
        if (b) break;
      }
      for (g = 2; ; ++g) {
        T k = g; bool b = true;
        for (int j = 0; j < n; k = k * k % P, ++j)
          if (k == 1) { b = false; break; }
        if (b && k == 1) break;
      }
    }

    container operator () (const container& a, const container& b) const {
      size_type n = StripLeadingZero(a), m = StripLeadingZero(b);
      ...
      T *l = u.data(), *r = v.data();
      trans(l, s), trans(r, s);
      T i_s = csl::pow(T(1), T(s), P - 2, P);
      for(int i = 0; i < s; ++i) l[i] = l[i] * i_s % P * r[i] % P;
      std::reverse(l + 1, l + s);
      trans(l, s), u.resize(d);
      return std::move(u);
    }

    void
    trans(T a[], int n) const {
